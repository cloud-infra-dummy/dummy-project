...
public abstract class TestSNAbstractGeneric {
    ...
    public void setUp() throws Exception {
        ...
        if (DAOFactory.isMock(accountDAO)) {
            doReturn(null).when(accountDAO).findByUserName(anyString());
            doReturn(m1).when(accountDAO).findByUserName(m1.getUserName());
            doReturn(m2).when(accountDAO).findByUserName(m2.getUserName());
            doReturn(m3).when(accountDAO).findByUserName(m3.getUserName());
            doReturn(m4).when(accountDAO).findByUserName(m4.getUserName());
        }
    }
    ...
    public void canListMembers() throws NoUserLoggedInException,
            UserNotFoundException {
        if (DAOFactory.isMock(accountDAO)) {
            HashSet<Account> mockAllMembers = new HashSet<>(Arrays.asList(m2, m3));
            doReturn(mockAllMembers).when(accountDAO).findAll();
        }
        sn.login(m1);
    ...
    public void canLeaveSocialNetwork() throws UserNotFoundException,
        ...
        if (DAOFactory.isMock(accountDAO)) {
            doReturn(null).when(accountDAO).findByUserName(m2.getUserName());
        }
        sn.login(m3);
        assertFalse(sn.hasMember(m2.getUserName()));
    }
	...
	public void testLogout() throws UserNotFoundException {
		sn.login(m2);
		sn.logout();
		assertThrows(NoUserLoggedInException.class, () -> sn.listMembers());
	}

    @Test
    public void testAutoAccepts() {
        assertFalse(m1.autoAccepts());
        m1.autoAcceptFriendships();
        assertTrue(m1.autoAccepts());
    }

    @Test
    public void testAccountDAO(){
        AccountDAO accountDAO = (AccountDAO) DAOFactory.getInstance().getAccountDAO();
        assertFalse(accountDAO.isConnectedToDB());
        accountDAO.save(m1);
        accountDAO.findByUserName("m1");
        accountDAO.delete(m1);
        accountDAO.update(m1);
        accountDAO.findAll();
    }
}
